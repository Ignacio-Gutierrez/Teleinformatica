...
def myNetwork():
    ...
    r1, r2, r3, r4, r5, r6 = [
        net.addHost('r{}'.format(i+1), ip='192.168.100.{}/29'.format(1+(i)*8))
        for i in range(6)]
    
    info( '*** Add switches\n')
    s1_a, s1_b, s2_a, s2_b, s3_a, s3_b, s4_a, s4_b, s5_a, s5_b, s6_a, s6_b = [
        net.addSwitch(s, cls=OVSKernelSwitch, failMode='standalone')
        for s in ('s1_a', 's1_b', 's2_a', 's2_b', 's3_a', 's3_b', \
                  's4_a', 's4_b', 's5_a', 's5_b', 's6_a', 's6_b')]

    info( '*** Add hosts\n')
    h1, h2, h3, h4, h5, h6 = [
        net.addHost('h{}'.format(i), cls=Host, ip='10.0.{}.254/24'.format(i), defaultRoute='via 10.0.{}.1'.format(i)) 
        for i in range(1, 7)]
